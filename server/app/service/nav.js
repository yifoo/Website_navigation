...
class NavService extends Service {
  ...
  async fetchSorts(params) {
    try {
      // @ts-ignore
      const sortList = await this.mysqlApp.query(
  ...
  async fetchSites(params) {
    try {
      console.time('site')
      // @ts-ignore
      const siteInfo = await this.mysqlApp
  ...
  async fetchTags(params) {
    try {
      // @ts-ignore
      // 获得一个
      const tagsList = await this.mysqlApp.query(`select tags from nav_sites where uid=${params.uid} group by tags`)
      let list =
        tagsList.length &&
        tagsList.map((item) => {
          if (item.tags) {
  ...
  async findSort(params) {
    try {
      // @ts-ignore
      const sortInfo = await this.mysqlApp.select('nav_sort', {
        // 搜索 post 表
  ...
  async findSite(params) {
    try {
      // @ts-ignore
      const siteInfo = await this.mysqlApp.select('nav_sites', {
        // 搜索 post 表
        where: params, // WHERE 条件
  ...
  async ClickSites(params) {
    try {
      // @ts-ignore
      const siteInfo = await this.mysqlApp.query(
  ...
  async getLogo(params) {
    try {
      // @ts-ignore
      const logoSrc = await this.mysqlApp.select('nav_logo', {
        // 搜索 post 表
