...
class UsersController extends Controller {
  ...
  *doLogin() {
    ...
    try {
      ...
      result = result[0]
      const func = yield app.mysql.query(
        `select fid from nav_role where rid=${result.rid}`
      )
      Object.keys(func).forEach((key) => {
        funcList.push(func[key].fid)
      })
      const date = new Date()
      result.login_time = `${date.getFullYear()}-${
        date.getMonth() + 1
      }-${date.getDate()} ${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}`
      yield app.mysql.query(
  ...
  *getUserInfo() {
    ...
    try {
      const func = yield app.mysql.query(
        `select fid from nav_role where rid=${user.rid}`
      )
      Object.keys(func).forEach((key) => {
        funcList.push(func[key].fid)
      })
  ...
  *register() {
    ...
    try {
      ...
      res = yield app.mysql.insert('nav_users', {
        uname,
        upwd,
        email,
        time,
      ...
      if (res.insertId) {
        if (isInit) {
          ...
          let initSites =
            'insert into nav_sites (sort_id,site_name,logo_id,site_url,site_desc,uid,order_index,is_com) values '
          ...
          let initSorts =
            'insert into nav_sort (sort_id,sort_name,parent_id,order_index,color,uid,is_com) values '
